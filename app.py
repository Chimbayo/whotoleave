...
def churn_summary():
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM customers WHERE prediction = 1")
        result = cursor.fetchone()

        if result and result[0] is not None:
            total_churners = result[0]
        else:
            total_churners = 0

        total_churners = max(total_churners, 1)  # avoid divide by zero
        ...
        for attr in attributes:
            if attr == 'proximity_to_nearestbranch_or_atm_km':
                query = f"""
                    SELECT
                        CASE
                            WHEN {attr} BETWEEN 0.2 AND 16 THEN '0.2 - 16 km'
                            WHEN {attr} BETWEEN 17 AND 32 THEN '17 - 32 km'
                            WHEN {attr} BETWEEN 33 AND 50 THEN '33 - 50 km'
                            ELSE 'Unknown'
                        END AS value,
                        COUNT(*) AS churners,
                ...
                query = f"""
                    SELECT
                        COALESCE(CAST({attr} AS TEXT), 'Unknown') AS value,
                        COUNT(*) AS churners,
                        ROUND(COUNT(*) * 100.0 / %s, 2) AS percentage
                    FROM customers
                    WHERE prediction = 1
                    GROUP BY COALESCE(CAST({attr} AS TEXT), 'Unknown')
                    ORDER BY percentage DESC
                """
                cursor.execute(query, (total_churners,))
