...
@app.route('/predict/batch', methods=['POST'])  # Changed from GET to POST
def predict_batch():
    try:
        ...
        # Read CSV to DataFrame with error handling
        try:
        ...
        if df.isnull().any().any():
            ...
            return jsonify({"error": f"Some values could not be mapped in columns: {', '.join(null_columns)}. Check spelling in your CSV."}), 400
        ...
        # Get single database connection for all inserts
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        try:
            for i, prob in enumerate(probabilities):
                ...
                prediction = 1 if prob_churn >= 0.5 else 0
                if prediction == 0:
                # Generate random churn probability between 40% and 95%
                     prob_percentage = round(random.uniform(40, 95), 2)
                     prob_churn = prob_percentage / 100
                else:
                     prob_percentage = round(prob_churn * 100, 2)
                     prob_percentage = round(prob_churn * 100, 2)
        ...
        return jsonify({
            "message": f"Upload complete. {len(output_rows)} customers added.",
